...
signed main() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        if(r > l1)
            seg[i].push_back({acos(l1 / r) * -1 - a, acos(l1 / r) - a});
        if(r > l2)
            seg[i].push_back({pi - acos(l2 / r) - a, pi + acos(l2 / r) - a});
//        for(auto j : seg[i])
//            cout << (j.first + a) / pi * 180 << " " << (j.second + a) / pi * 180 << endl;
//        cout << endl;
...
//        cout << i.first << " " << i.second << endl;
    ...
    sum += r - l;
    cout << fixed << setprecision(12) << 360.0 - sum / pi * 180 << endl;
    return 0;
}
